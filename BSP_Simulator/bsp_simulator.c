// BSP Simulator Implementation
// SDL2-based implementation of the BSP API for host-side simulation
// This allows the exact same App/ code to run on both STM32 and simulator
...
typedef struct {
    ...
    // Input state
    bool buttons_pressed[BSP_BUTTON_COUNT];
    bsp_button_event_t event_queue[32];
    int event_queue_head, event_queue_tail, event_queue_count;
    ...
    bool memory_wire_heating;
    uint32_t memory_wire_start_time;
    uint16_t memory_wire_duration;
    
    // Power management
    bsp_power_mode_t power_mode;
    int sleep_suppression_count;
...
int bsp_input_init(void) {
    // Clear button states
    memset(g_sim_state.buttons_pressed, 0, sizeof(g_sim_state.buttons_pressed));
    g_sim_state.event_queue_head = 0;
    g_sim_state.event_queue_tail = 0;
    g_sim_state.event_queue_count = 0;
    
    return 0;
}

void bsp_input_cleanup(void) {
    // Nothing to cleanup for input
}

static void add_button_event(bsp_button_id_t button, bool pressed) {
    if (g_sim_state.event_queue_count >= 32) {
        return; // Queue full
    }
    
    bsp_button_event_t* event = &g_sim_state.event_queue[g_sim_state.event_queue_tail];
    event->button = button;
    event->pressed = pressed;
    event->timestamp = bsp_get_tick_ms();
    
    g_sim_state.event_queue_tail = (g_sim_state.event_queue_tail + 1) % 32;
    g_sim_state.event_queue_count++;
}
...
bool bsp_input_poll_event(bsp_button_event_t* event) {
    ...
    while (SDL_PollEvent(&sdl_event)) {
        if (sdl_event.type == SDL_QUIT) {
            ...
            if (button_id < BSP_BUTTON_COUNT) {
                ...
                if (g_sim_state.buttons_pressed[button_id] != pressed) {
                    g_sim_state.buttons_pressed[button_id] = pressed;
                    add_button_event(button_id, pressed);
                }
            }
        }
    }
    ...
    if (g_sim_state.event_queue_count > 0) {
        *event = g_sim_state.event_queue[g_sim_state.event_queue_head];
        g_sim_state.event_queue_head = (g_sim_state.event_queue_head + 1) % 32;
        g_sim_state.event_queue_count--;
        return true;
    }
...
// =============================================================================
// RTOS SIMULATION (Simplified)
// =============================================================================

// For simulator, we'll implement a simple cooperative scheduler
// This is a simplified RTOS simulation - real STM32 will use FreeRTOS

typedef struct {
    bsp_task_function_t function;
    void* parameters;
    const char* name;
    uint8_t priority;
    bool active;
...
static sim_task_t sim_tasks[8];
static int sim_task_count = 0;
...
bsp_task_handle_t bsp_task_create(bsp_task_function_t task_function, 
                                  ...
                                  void* parameters,
                                  uint8_t priority) {
    (void)stack_size; // Not used in simulation
    
    if (sim_task_count >= 8) {
        return NULL;
    }
    
    sim_task_t* task = &sim_tasks[sim_task_count];
    ...
    printf("Created task: %s (priority %d)\n", name, priority);
    return (bsp_task_handle_t)task;
}

void bsp_task_delete(bsp_task_handle_t task) {
    // Simple implementation - mark as inactive
    sim_task_t* sim_task = (sim_task_t*)task;
    if (sim_task) {
        sim_task->active = false;
    }
}
...
void bsp_scheduler_start(void) {
    printf("Starting RTOS scheduler simulation with %d tasks\n", sim_task_count);
    g_sim_state.scheduler_running = true;
    
    // Start all tasks as threads
    for (int i = 0; i < sim_task_count; i++) {
        if (sim_tasks[i].active) {
            if (pthread_create(&sim_tasks[i].thread, NULL, task_wrapper, &sim_tasks[i]) != 0) {
                printf("ERROR: Failed to create thread for task %s\n", sim_tasks[i].name);
                return;
            }
        }
    }
    
    // Wait for all tasks to complete (they shouldn't, they're infinite loops)
    for (int i = 0; i < sim_task_count; i++) {
        if (sim_tasks[i].active) {
            pthread_join(sim_tasks[i].thread, NULL);
        }
    }
...
bsp_queue_handle_t bsp_queue_create(uint8_t length, uint8_t item_size) {
    sim_queue_t* queue = malloc(sizeof(sim_queue_t));
...
void bsp_queue_delete(bsp_queue_handle_t queue) {
    sim_queue_t* sim_queue = (sim_queue_t*)queue;
    if (sim_queue) {
        free(sim_queue->buffer);
        free(sim_queue);
    }
}

bool bsp_queue_send(bsp_queue_handle_t queue, const void* item, uint32_t timeout_ms) {
    (void)timeout_ms; // Simplified - no timeout handling
    
    sim_queue_t* sim_queue = (sim_queue_t*)queue;
    if (!sim_queue || sim_queue->count >= sim_queue->length) {
        return false;
    }
    
    uint8_t* dest = sim_queue->buffer + (sim_queue->tail * sim_queue->item_size);
    memcpy(dest, item, sim_queue->item_size);
    
    sim_queue->tail = (sim_queue->tail + 1) % sim_queue->length;
    sim_queue->count++;
...
bool bsp_queue_receive(bsp_queue_handle_t queue, void* item, uint32_t timeout_ms) {
    ...
    sim_queue_t* sim_queue = (sim_queue_t*)queue;
    if (!sim_queue || sim_queue->count == 0) {
        return false;
    }
    
    uint8_t* src = sim_queue->buffer + (sim_queue->head * sim_queue->item_size);
    memcpy(item, src, sim_queue->item_size);
    
    sim_queue->head = (sim_queue->head + 1) % sim_queue->length;
    sim_queue->count--;
...
void bsp_debug_trigger_button(bsp_button_id_t button) {
    if (button < BSP_BUTTON_COUNT) {
        add_button_event(button, true);
        add_button_event(button, false);
